...
export default function Model({ activeMenu }) {
  ...
  const [textures, setTextures] = useState(projects.map(() => null)); // Initialize textures state

  // Load textures outside of useEffect
  const loadedTextures = projects.map(project => useTexture(project.src));

  useEffect(() => {
    setTextures(loadedTextures); // Update textures state when activeMenu changes
  }, [activeMenu])
  ...
  }, [activeMenu, textures]) // Update dependencies to include textures
